...
```mermaid
sequenceDiagram
  Application->>extmod-resolver: import "https..."
  extmod-resolver-->>Application: Existing max-age TTL?
  extmod-resolver->>Remote Module: HEAD
  Remote Module->>extmod-resolver: HEAD response
  extmod-resolver-->>Application: Existing etag?
  alt response has etag
      extmod-resolver-->>extmod-loader: Module + etag
  else response has Cache-Control
      extmod-resolver-->>extmod-loader: Module + max-age TTL
  else
      extmod-resolver-->>extmod-loader: Module (bare)
  end
  extmod-loader->>Remote Module: GET
  Remote Module->>extmod-loader: GET response
  extmod-loader->>Application: Load Module
```
